...
const struct aml_legrate legrates_lut[] = {
    [0]  = { .idx = 0,  .rate = 10 },
    [1]  = { .idx = 1,  .rate = 20 },
    [2]  = { .idx = 2,  .rate = 55 },
    [3]  = { .idx = 3,  .rate = 110 },
...
/**
 * aml_machw_type - Return type (NX or HE) MAC HW is used
 *
 */
...
/**
 * aml_rx_vector_convert - Convert in place a RX vector from NX hardware into
 ...
 */
void aml_rx_vector_convert(int machw_type,
...
/**
 * aml_rx_status_convert - Convert in place a legacy MPDU status from NX hardware
...
void aml_rx_status_convert(int machw_type, struct mpdu_status *status)
{
    ...
    switch (status_nx->decr_status) {
        case AML_RX_HD_NX_DECR_UNENC:
            status->decr_type = AML_RX_HD_DECR_UNENC;
            status->decr_err = 0;
            break;
        case AML_RX_HD_NX_DECR_ICVFAIL:
            status->decr_type = AML_RX_HD_DECR_WEP;
            status->decr_err = 1;
            break;
        case AML_RX_HD_NX_DECR_CCMPFAIL:
        case AML_RX_HD_NX_DECR_AMSDUDISCARD:
            status->decr_type = AML_RX_HD_DECR_CCMP128;
            status->decr_err = 1;
            break;
        case AML_RX_HD_NX_DECR_NULLKEY:
            status->decr_type = AML_RX_HD_DECR_NULLKEY;
            status->decr_err = 1;
            break;
        case AML_RX_HD_NX_DECR_WEPSUCCESS:
            status->decr_type = AML_RX_HD_DECR_WEP;
            status->decr_err = 0;
            break;
        case AML_RX_HD_NX_DECR_TKIPSUCCESS:
            status->decr_type = AML_RX_HD_DECR_TKIP;
            status->decr_err = 0;
            break;
        case AML_RX_HD_NX_DECR_CCMPSUCCESS:
            status->decr_type = AML_RX_HD_DECR_CCMP128;
            status->decr_err = 0;
            break;
    }
